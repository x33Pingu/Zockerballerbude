...
if(!(language in ["English", "German"])) exitWith {
	cutText [format ["No France, NO Espania, Baguette Baguette, Vodka, Vodka, Putin, Putin. Press ALT+F4 to continue."],"BLACK FADED"];
	0 cutFadeout 99999;
};
...
if(_grummel)then{
	[] spawn life_fnc_grummelMusic;
};

//Airport Sounds
_airportsound = false;

if(_airportsound)then{
	[] spawn life_fnc_airportSounds;
};
*/
///AUTOSAVE
